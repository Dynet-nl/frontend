 ...
 */
const extractFloorFromFlat = (flat: Flat): number => {
    // Null safety check
 ...
 */
const extractFloorFromIdentifier = (identifier: string): number => {
 ...
 */
const calculateBuildingFloors = (flats: Flat[]): number => {
    ...
    if (totalFlats === 2) {
        ...
        const indicators = flats.map(f => f.toevoeging?.trim()).filter(Boolean) as string[];
 ...
 */
export const categorizeBuilding = (flats: Flat[]): CategorizationResult => {
 ...
 */
export const generateHBNumber = (building: Building, flats: Flat[]): string => {
