...
const HASInstallerAgendaCalendarPage: React.FC = () => {
    const axiosPrivate = useAxiosPrivate();
    const navigate = useNavigate();
    const [events, setEvents] = useState<CalendarEvent[]>([]);
    const [originalEvents, setOriginalEvents] = useState<CalendarEvent[]>([]);
    const [loading, setLoading] = useState<boolean>(false);
    const [selectedHASMonteur, setSelectedHASMonteur] = useState<string>('');
    ...
    const fetchHasMonteurs = useCallback(async (): Promise<void> => {
        try {
            ...
            const monteurs = users.filter((user) => {
                ...
                const isHASMonteur =
                    hasMonteur && (hasMonteur === 2023 || hasMonteur === true || hasMonteur === 1 || (typeof hasMonteur === 'number' && hasMonteur > 0));
            ...
            const fallbackUsers: HASMonteur[] = [
    ...
    const fetchAppointments = useCallback(async (): Promise<void> => {
        try {
            setLoading(true);
            const response = await axiosPrivate.get<FlatAppointment[]>('/api/apartment/appointments/all-hasmonteur', {
                params: {
                    limit: 500,
                },
            });
            ...
            const calendarEvents: CalendarEvent[] = response.data
                ...
                .map((flat) => {
                    ...
                    const endTimeParts = appointmentData.endTime
    ...
    const handleHASMonteurFilter = (e: ChangeEvent<HTMLSelectElement>): void => {
        ...
        if (!selectedName) {
            ...
            const filteredEvents = originalEvents.filter((event) => {
                ...
                return eventPersonName.toLowerCase().includes(selectedName.toLowerCase()) || eventPersonName === selectedName;
            });
                ...
                <ConfirmModal
                    ...
                    message={
                        <div style={{ textAlign: 'left', lineHeight: '1.8' }}>
                            <p>
                                <strong>🏠 Address:</strong> {eventDetailsModal.event.resource?.address}
                            </p>
                            <p>
                                <strong>🔧 Type:</strong> {eventDetailsModal.event.resource?.type}
                            </p>
                            <p>
                                <strong>📝 Notes:</strong> {eventDetailsModal.event.resource?.notes || 'None'}
                            </p>
                            <p>
                                <strong>🏢 Complex:</strong> {eventDetailsModal.event.resource?.complexNaam}
                            </p>
                            <p>
                                <strong>👷 Installer:</strong> {eventDetailsModal.event.personName || 'Not assigned'}
                            </p>
                            <p>
                                <strong>⏱️ Duration:</strong> {eventDetailsModal.event.duration || 'Not specified'} minutes
                            </p>
                        </div>
                    }
