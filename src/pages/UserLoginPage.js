...
const UserLoginPage = () => {
    ...
    const [loginStage, setLoginStage] = useState(LOGIN_STAGES.IDLE)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!err?.response) {
                ...
                setErrMsg(`❌ Login Failed - ${err.message || 'An unexpected error occurred.'}`)
            }
    ...
    const getStageColor = () => {
        switch (loginStage) {
            case LOGIN_STAGES.ERROR: return '#dc3545'
            case LOGIN_STAGES.SUCCESS:
            case LOGIN_STAGES.REDIRECTING: return '#28a745'
                        ...
                        {isLoading && (
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    width: loginStage === LOGIN_STAGES.VALIDATING ? '20%' :
                                           loginStage === LOGIN_STAGES.CONNECTING ? '40%' :
                                           loginStage === LOGIN_STAGES.AUTHENTICATING ? '60%' :
                                           loginStage === LOGIN_STAGES.LOADING_PROFILE ? '80%' :
                                           loginStage === LOGIN_STAGES.REDIRECTING ? '95%' : '100%',
