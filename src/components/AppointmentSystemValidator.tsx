...
interface ValidationResults {
    technicalAppointments?: ValidationCheck[];
    hasAppointments?: ValidationCheck[];
    appointmentDisplay?: ValidationCheck[];
    calendarIntegration?: ValidationCheck[];
    dataConsistency?: ValidationCheck[];
    error?: string;
}

interface TechnicalAppointment {
    technischePlanning?: {
        appointmentBooked?: {
            date?: string;
            startTime?: string;
...
const AppointmentSystemValidator: React.FC = () => {
    ...
    const validateAppointmentSystem = async (): Promise<void> => {
        setLoading(true);
    ...
    const validateTechnicalAppointments = async (): Promise<ValidationCheck[]> => {
    ...
    const validateHASAppointments = async (): Promise<ValidationCheck[]> => {
        const checks: ValidationCheck[] = [];
    ...
    const validateAppointmentDisplay = async (): Promise<ValidationCheck[]> => {
        const checks: ValidationCheck[] = [];
    ...
    const validateCalendarIntegration = async (): Promise<ValidationCheck[]> => {
        const checks: ValidationCheck[] = [];
    ...
    const validateDataConsistency = async (): Promise<ValidationCheck[]> => {
        const checks: ValidationCheck[] = [];
    ...
    // ISO 8601 week number calculation (correct method)
    const calculateWeekNumber1 = (date: Date): number => {
    ...
    // Using same correct calculation for consistency
    const calculateWeekNumber2 = (date: Date): number => {
        const d = new Date(date);
        d.setHours(0, 0, 0, 0);
        d.setDate(d.getDate() + 4 - (d.getDay() || 7));
        const yearStart = new Date(d.getFullYear(), 0, 1);
